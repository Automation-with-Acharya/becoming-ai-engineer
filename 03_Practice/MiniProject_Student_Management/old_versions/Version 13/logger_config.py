...
- StreamHandler (console) level is now controlled by LOG_LEVEL in .env
  (Day 019 Exercise 6) — defaults to INFO so the terminal stays readable.
- The Formatter includes timestamp, level, module name, and the message —
  giving full context in every log line.

Day 017 Exercise 1: logging.basicConfig equivalent → replaced by manual Logger setup
                    for finer-grained control (separate console vs file levels).
Day 017 Exercise 2: Write logs to logs/application.log (FileHandler).
Day 017 Exercise 3: Replace print() with logger.info() / logger.error() / logger.warning().
Day 017 Exercise 4: Log exceptions inside try/except blocks with logger.exception().
Day 017 Exercise 5: Review output on both console and log file.
Day 019 Exercise 6: Console log level driven by LOG_LEVEL env var via config.settings.
"""

import logging
import os

# Day 019 Exercise 6: Import settings to read LOG_LEVEL from .env
# Import is deferred-safe — config.py is a plain module with no circular deps.
