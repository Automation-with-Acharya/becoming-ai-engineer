...
Day 022 Update: get_student_by_email() added to both the abstract interface and the concrete
                implementation. It issues a WHERE email = %s point lookup which is served by
                idx_students_email (UNIQUE B-Tree) in O(log N) — no full-table scan.
"""
...
class StudentRepository(ABC):
    ...
    @abstractmethod
    def get_student_by_email(self, email: str) -> Student_response_model | None:
        """
        Retrieve a student by their unique email address.

        Day 022: This lookup is served by idx_students_email — a UNIQUE B-Tree index
        created on startup. The WHERE email = %s predicate matches the index exactly,
        turning the search into an O(log N) point lookup instead of an O(N) full scan.
...
class PostgresStudentRepository(StudentRepository):
    ...
    def add_student(self, student: Student_model) -> Student_response_model:
        ...
        try:
            ...
            def _create(cur):
                ...
                cur.execute(
                    "INSERT INTO students (id, name, age, city, email, is_active) VALUES (%s, %s, %s, %s, %s, %s)",
                    (assigned_id, student.name, student.age, student.city, student.email, student.is_active),
                )
                return assigned_id
    ...
    def get_all_students(self) -> list[Student_response_model]:
        ...
        try:
            # rows = self.db_helper.fetch_all("SELECT id, name, age, city, email, is_active FROM students ORDER BY id")
            rows = self.db_helper.fetch_all("SELECT id, name, age, city, email, is_active FROM small_students ORDER BY id")
            logger.info("Repository: Retrieved %d student(s).", len(rows))
            return [
                Student_response_model(
                    id=row[0], name=row[1], age=row[2], city=row[3], email=row[4], is_active=row[5]
                )
                for row in rows
            ]
        except Exception:
    ...
    def get_student_by_id(self, student_id: int) -> Student_response_model | None:
        ...
        try:
            row = self.db_helper.fetch_one(
                "SELECT id, name, age, city, email, is_active FROM students WHERE id = %s",
                (student_id,),
            )
            if row is None:
    ...
    def search_students(self, name_input: str) -> list[Student_response_model]:
        ...
        try:
            rows = self.db_helper.fetch_all(
                "SELECT id, name, age, city, email, is_active FROM students WHERE name ILIKE %s ORDER BY id",
                (f"%{name_input}%",),
            )
            logger.info(
    ...
    def get_student_by_email(self, email: str) -> Student_response_model | None:
        ...
        Day 022: PostgreSQL uses idx_students_email (UNIQUE B-Tree on students.email)
        to resolve this query. Instead of scanning every row, it traverses the B-Tree
        in O(log N) steps:
          - N = 10,000 students → ~14 node visits (log₂ 10000 ≈ 13.3)
          - N = 1,000,000 students → ~20 node visits (log₂ 1000000 ≈ 19.9)
        This makes email-based lookups (e.g., login, deduplication) extremely fast at scale.

        Args:
            email (str): The email address to look up (exact match, case-insensitive
                         because PostgreSQL VARCHAR comparison is case-sensitive by default;
                         callers should normalise to lowercase before passing in).

        Returns:
            Student_response_model | None: Student instance if found, otherwise None.
        """
        logger.debug("Repository: Looking up student with email='%s'.", email)
        try:
            row = self.db_helper.fetch_one(
                "SELECT id, name, age, city, email, is_active FROM students WHERE email = %s",
                (email,),
            )
            if row is None:
                logger.warning("Repository: Student with email='%s' not found.", email)
                return None
            logger.debug(
                "Repository: Found student id=%d for email='%s'.", row[0], email
            )
            return Student_response_model(
                id=row[0],
                name=row[1],
            ...
            logger.exception(
                "Repository: Failed to look up student with email='%s'.", email
            )
            raise
