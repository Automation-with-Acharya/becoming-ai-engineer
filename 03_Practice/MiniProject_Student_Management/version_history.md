...
**When:** Day 037  
**Theme:** Connect the React frontend to the FastAPI backend. Replace mock data with live API calls using `useEffect`, `fetch`, and a dedicated API module. Implement loading, error, and empty states. Configure the Vite development proxy. Understand CORS in the full-stack context.

### What Changed

| Area | Before (v26) | After (v27) |
| ---- | ------------ | ----------- |
| **`src/api/studentApi.js`** | — | NEW: HTTP boundary module with `getStudents()` and `getStudentById()` |
| **`src/App.jsx`** | Mock data import, static render | REPLACED: `useEffect` fetch, `loading` / `error` / `students` state, four explicit UI states |
| **`src/App.css`** | Layout + card styles | + `.status-message` variants for loading, error, and empty states |
| **`vite.config.js`** | No proxy | + `server.proxy`: `/api/*` → `http://localhost:8000/*` (strips `/api` prefix) |
| **`src/components/StudentCard.jsx`** | Crashed on missing `active` field | Handles `active === undefined` gracefully; shows "Status N/A" badge |
| **`src/components/Header.jsx`** | "Day 036" subtitle | "Day 037 Integration" subtitle |
| **`src/components/Footer.jsx`** | "Day 036" subtitle | "Day 037 Integration" subtitle |
| **`src/data/mockStudents.js`** | Imported by App | Kept for reference; no longer imported |
| **`main.py` (CORS)** | Already configured | No change needed — `allow_origins=["http://localhost:5173"]` was correct |
| **Exercises answer sheet** | — | Written: `Day037_React_FastAPI_API_Integration/Exercises_answer_sheet.md` |

### Why

1. **API module (Exercise 3):** Inline `fetch()` in components violates separation of concerns — the component would need to know about HTTP, URLs, and error handling simultaneously. `studentApi.js` is the frontend's Repository: one place to change base URLs, add auth headers, or centralise error handling. Matches the `StudentRepository` pattern already built on the backend.

2. **useEffect + async (Exercise 7):** Data fetching is a side effect — it is external to the render cycle. `useEffect` is React's hook for running side effects after render. The inner async function pattern is required because `useEffect` cannot receive an async function directly (it would return a Promise instead of a cleanup function).

3. **Four UI states (Exercises 5, 6, 8):** Any data-fetching UI has exactly four states: Loading / Error / Empty / Data. Conflating any two leads to incorrect UI: showing "no students" while the request is still running, or showing nothing on error. Explicitly modelling all four states is a basic enterprise frontend pattern.

4. **Presentation layer unchanged (Exercise 9):** `StudentList` and `StudentCard` were not touched. This validates the Day 036 architecture decision to separate data management (App) from presentation (components). The components don't know or care whether data came from a mock array or an API — they just receive props.

5. **Vite proxy (Exercise 11):** The proxy routes `/api/*` requests from the browser (`:5173`) to FastAPI (`:8000`) inside Vite's dev server. The browser sees all traffic going to its own origin — no CORS preflight. The proxy does not replace the FastAPI `CORSMiddleware`; in production a reverse proxy or same-origin deployment handles routing.

### How

**studentApi.js — the API module boundary:**

```javascript
export async function getStudents() {
  const response = await fetch("/api/students");  // Vite proxy routes to :8000/students
...
**App.jsx — useEffect data fetch pattern:**
...
**App.jsx — four UI state rendering:**

```jsx
{loading && <p className="status-message--loading">⏳ Loading students…</p>}
{error   && <p className="status-message--error" role="alert">❌ {error}</p>}
{!loading && !error && students.length === 0 && <p className="status-message--empty">📭 No students found.</p>}
{!loading && !error && students.length > 0  && <><StudentSummary /><StudentList /></>}
```
...
```
frontend/student-management-ui/src/api/
    studentApi.js                               NEW: Exercise 3 — HTTP module
frontend/student-management-ui/src/
    App.jsx                                     REPLACED: Exercises 4–9
    App.css                                     + loading/error/empty styles
frontend/student-management-ui/
    vite.config.js                              + server.proxy (Exercise 11)
frontend/student-management-ui/src/components/
    StudentCard.jsx                             + active=undefined handling (Exercise 2/9)
    Header.jsx                                  subtitle updated
    Footer.jsx                                  subtitle updated
README.md                                       frontend structure + Day 037 feature section
version_history.md                              This entry
Day037_React_FastAPI_API_Integration/
    Exercises_answer_sheet.md                   Written: all 12 exercise answers
```

---

## v26 - Day 036: React + Vite Frontend Scaffold


**When:** Day 036  
**Theme:** Introduce the React + Vite frontend layer — component trees, props, mock data, conditional rendering, state, and the React event loop. All 11 exercises completed and documented.

### What Changed

| Area | Before (v25) | After (v26) |
| ---- | ------------ | ----------- |
| **`frontend/`** | Empty shell directory | NEW: `student-management-ui/` Vite scaffold (React 19, JavaScript) |
| **`index.html`** | Vite default boilerplate | Updated: Exercise 3 comments, improved SEO title + meta description |
| **`vite.config.js`** | Default, no comments | Updated: Exercise 3 entry-point explanation comment |
| **`src/main.jsx`** | Default, no comments | Updated: Exercise 3 — explains `createRoot`, `StrictMode`, #root mount |
| **`src/App.jsx`** | Vite default counter demo | **Replaced**: Exercises 3/4/5/8 — component tree root, state, mock data |
| **`src/App.css`** | Vite default demo styles | **Replaced**: Exercise 10 — functional layout styles for the student UI |
| **`src/index.css`** | Vite default reset | **Replaced**: simplified global reset + comments |
| **`src/data/mockStudents.js`** | — | NEW: Exercise 5 — 3 students, mirrors `Student_response_model` shape |
| **`src/components/Header.jsx`** | — | NEW: Exercise 4 — app title bar |
| **`src/components/Footer.jsx`** | — | NEW: Exercise 4 — bottom banner |
| **`src/components/StudentSummary.jsx`** | — | NEW: Exercise 9 — total/active/inactive derived stats |
| **`src/components/StudentList.jsx`** | — | NEW: Exercises 4/6/11 — maps array → StudentCard with `key={student.id}` |
| **`src/components/StudentCard.jsx`** | — | NEW: Exercises 6/7/8/11 — single record, Active/Inactive badge, click handler |
| **`Day036_React_&_Vite/Exercises_answer_sheet.md`** | Empty file | Written: full answers for all 11 exercises |

### Why

1. **Component composition (Exercise 4):** Understanding that UI = f(components) is the foundation of every React project. Each component has one job; `App` only orchestrates — it never renders individual student fields itself.

2. **Mock data before API (Exercise 5):** Connecting to a real backend before understanding props/state creates two moving parts simultaneously. A local array with the correct shape lets us learn React in isolation, then swap in the API on Day 037 with zero component changes.

3. **Backend model → React props chain (Exercise 6):** `Student_response_model` in FastAPI defines the JSON shape; `mockStudents.js` mirrors it; `StudentCard` receives it as a prop. This intentional connection means the frontend understands the backend contract *before* the API call exists.

4. **Conditional rendering (Exercise 7):** Ternary expressions are React's primary conditional tool. `student.active ? <Active/> : <Inactive/>` reinforces that render output depends entirely on data — not on imperative DOM manipulation.

5. **The React loop (Exercise 8):** The mental model `User interaction → Event → State update → Re-render → UI change` is more important than memorising `useState` syntax. Demonstrating the loop with a real selection behaviour internalises this.

6. **Derivation over state (Exercise 9):** `StudentSummary` computes total/active/inactive from a prop with `filter + length`. No `useState` needed. This reinforces: if a value can be derived from existing state/props, it should not be stored in state.

7. **`key` prop identity (Exercise 11):** `key={student.id}` vs `key={student.name}` directly connects backend primary keys to React's reconciliation algorithm — a concrete demonstration of why DB identity matters on the frontend.
...
cd student-management-ui && npm install
```

**Component tree assembly (Exercise 4):**

```
App
├── Header           — presentational, no props
├── StudentSummary   — receives students[], derives stats
├── StudentList      — receives students[], onSelect; maps → StudentCard
│   └── StudentCard  — receives {student}, onSelect; renders one record
└── Footer           — presentational, no props
```

**State and prop flow (Exercise 8):**
...
// → StudentCard onClick → onSelect(student.id) → state update → re-render
```

**Conditional rendering (Exercise 7):**
...
**Key prop drill (Exercise 11):**

```jsx
// ❌ Wrong: key={student.name} — mutable, not guaranteed unique
// ✅ Right: key={student.id}  — stable DB primary key
students.map((s) => <StudentCard key={s.id} student={s} onSelect={onSelect} />)
```

### Where

```
frontend/student-management-ui/index.html           Updated: Exercise 3 comments, SEO meta
frontend/student-management-ui/vite.config.js       Updated: Exercise 3 entry-point comments
frontend/student-management-ui/src/main.jsx         Updated: Exercise 3 — createRoot explanation
frontend/student-management-ui/src/App.jsx          REPLACED: student management tree + state
frontend/student-management-ui/src/App.css          REPLACED: functional student UI styles
frontend/student-management-ui/src/index.css        REPLACED: global reset
frontend/student-management-ui/src/data/
    mockStudents.js                                 NEW: Exercise 5 mock data array
frontend/student-management-ui/src/components/
    Header.jsx                                      NEW: Exercise 4
    Footer.jsx                                      NEW: Exercise 4
    StudentSummary.jsx                              NEW: Exercise 9
    StudentList.jsx                                 NEW: Exercises 4/6/8/11
    StudentCard.jsx                                 NEW: Exercises 6/7/8/11
README.md                                           Updated: frontend structure, tech stack, run instructions
version_history.md                                  This entry
Day036_React_&_Vite/Exercises_answer_sheet.md       Written: all 11 exercise answers
```
