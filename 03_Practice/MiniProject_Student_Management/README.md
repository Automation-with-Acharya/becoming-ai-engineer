...
**Docker Compose Configuration (Day 027)**

- 📋 **Explicit `environment:` Block** — `api` service lists all variables with `${VAR}` interpolation; each key is individually visible and overridable without touching `.env`
- 🔼 **Precedence Rule** — `environment:` key wins over the same key from `env_file`; `env_file` wins over image defaults — Compose resolves deterministically
- 🔄 **`restart: on-failure`** — Docker automatically restarts the `api` container on any non-zero exit code (crash, OOM); clean exits (`code 0`) are not restarted
- 🔎 **`docker compose config`** — prints the fully-resolved YAML after all `${VAR}` substitutions; use to audit exactly what reaches each container
- ⚗️ **Dev vs Prod Bind Mount** — source-code bind mount useful in dev (instant code changes without rebuild); removed in prod to keep the image immutable and reproducible

**Docker Internal Networking (Day 028)**

- 🌐 **Auto-created Bridge Network** — Compose creates `miniproject_student_management_default` (driver: `bridge`, subnet `172.18.0.0/16`) automatically; both services join it
- 🔤 **Service-Name DNS** — Docker's embedded DNS server (`127.0.0.11`) resolves service names: `db` → `172.18.0.2`, `api` → `172.18.0.3`; IPs are dynamic, names are stable
- 🚫 **`localhost` ≠ another container** — `localhost` inside `api` resolves to the api container's own loopback (`127.0.0.1`), NOT the db container; `DB_HOST` must be `db`, never `localhost`
...
- 🔌 **API Module** — `src/api/studentApi.js` centralises all HTTP calls; components never call `fetch()` directly; mirrors the Repository Pattern used on the backend (Exercise 3)
- 🔄 **Mock Data Replaced** — `App.jsx` no longer imports `mockStudents.js`; live data from `GET /students` via `getStudents()` drives the UI; `mockStudents.js` kept for reference only (Exercise 4)
- ⏳ **Loading State** — `useState(true)` for `loading`; renders "⏳ Loading students…" while the request is in-flight; loading and empty are treated as distinct states (Exercise 5)
- ❌ **Error State** — `catch (err) { setError(err.message) }`; error renders with `role="alert"` for screen-reader announcement; preserves actual error message for dev debugging (Exercise 6)
- ⚙️ **useEffect Mount Fetch** — `useEffect(() => { loadStudents() }, [])` fires once after first render; inner async function pattern avoids returning a Promise from the effect callback (Exercise 7)
- 📫 **Empty State** — explicit fourth UI state: `!loading && !error && students.length === 0` renders "📭 No students found" (Exercise 8)
- 🎨 **Presentation Layer Unchanged** — `StudentList → StudentCard` required zero rewrites; only the data source changed; this validates the component boundary from Day 036 (Exercise 9)
- 🌐 **CORS Verified** — `CORSMiddleware` in `main.py` already correctly configured for `http://localhost:5173`; `allow_origins=["*"]` avoided for security (Exercise 10)
- 🔀 **Vite Dev Proxy** — `vite.config.js` proxy routes `/api/*` → `http://localhost:8000/*` (strips `/api` prefix); eliminates browser CORS preflight in dev; does NOT replace production CORS (Exercise 11)
- 🔍 **Network Debugging** — DevTools Network tab traced: Browser → Vite proxy → FastAPI → JSON response → `setStudents` → UI re-render (Exercise 12)

**React + Vite Frontend (Day 036)**

- ⚛️ **Vite Scaffold** — `npm create vite@latest student-management-ui` with React + JavaScript; served by Vite dev server (`npm run dev`) with HMR
- 🌳 **Component Tree** — `App → Header, StudentSummary, StudentList → StudentCard, Footer`; each component has a single responsibility (Exercise 4)
- 📦 **Mock Student Data** — `src/data/mockStudents.js` mirrors the `Student_response_model` shape (`id, name, age, city, email, active`); replaced by real API in Day 037 (Exercise 5)
- 🃏 **StudentCard** — renders one student inside `<article>`; demonstrates the Backend model → JSON shape → React props → Component chain (Exercise 6)
- 🔀 **Conditional Rendering** — `student.active` ternary renders `✅ Active` or `❌ Inactive` badge (Exercise 7)
- 🔁 **React State Loop** — `useState(null)` in App tracks `selectedStudentId`; click → `onSelect(id)` → `setSelectedStudentId` → re-render → selected banner updates (Exercise 8)
- 📊 **StudentSummary** — derives total/active/inactive counts from the `students` prop array using `filter` + `length`; no state needed (Exercise 9)
- 🎨 **Clean First Screen** — header, stats bar, card grid, selected-student banner, footer — all rendered with minimal functional CSS (Exercise 10)
- 🔑 **key Prop Drill** — `key={student.id}` (stable DB primary key) vs `key={student.name}` (mutable display value); reinforces backend identity → frontend reconciliation (Exercise 11)
...
| Technology             | Purpose                                                                              |
| ---------------------- | ------------------------------------------------------------------------------------ |
| **React 19**           | UI component library — declarative rendering, state, props, conditional rendering    |
| **Vite 8**             | Build tool and dev server — HMR, ES module bundling, JSX transform                  |
| **JavaScript (ES6+)**  | Language — no TypeScript for Day 036 per course spec                                 |
| **@vitejs/plugin-react** | Babel JSX transform + React Fast Refresh for instant dev feedback                  |
| **oxlint**             | Fast JavaScript linter (Vite default, replaces ESLint)                               |
| **Vanilla CSS**        | Component styles — no framework; deliberate to keep focus on React concepts          |

---

## 🚀 How to Run

### Run the Frontend (Day 036)

> Node.js `22.12+` required (confirmed: `v22.22.3`).

```bash
cd frontend/student-management-ui
npm install        # install React, Vite, and dev deps (already done on scaffold)
npm run dev        # starts Vite dev server at http://localhost:5173
```

Vite serves with **HMR** — any file save instantly updates the browser without a full reload.
